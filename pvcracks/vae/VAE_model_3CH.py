...
class Encoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(3, 32, kernel_size=7, stride=2, dilation=1, padding=3),  # Modified for 3CH
            nn.ReLU(),
...
class Decoder(nn.Module):
    def __init__(self, latent_dim):
        ...
        self.fc = nn.Linear(latent_dim, 1024 * 7 * 7)
        self.deconv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.ConvTranspose2d(32, 3, kernel_size=7, stride=2, dilation=1, padding=3, output_padding=1),  # Modified for 3CH
            nn.Sigmoid()
        )
