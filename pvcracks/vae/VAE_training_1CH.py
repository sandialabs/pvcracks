...
transform = transforms.Compose(
    [FixedRotation(angle=180), FixedHorizontalFlip(), FixedVerticalFlip()]
)
...
train_loader = DataLoader(
    train_augmented, batch_size=batch_size, shuffle=True, num_workers=1
)
        ...
        if epoch_loss >= train_losses[-25]:
            if epoch_loss >= train_losses[-30]:
                if epoch_loss >= train_losses[-35]:
                    sys.exit(
                        "Training loss stuck, Overfitting. Current loss %f > loss 25, 30 & 35 epochs ago %f"
                        % (epoch_loss, train_losses[-25])
                    )
...
show_input_output_images(
    images, vae_outputs, num_images_to_display, path="CurrentResults/"
)
num_images_to_compare = 99
ssim_comp = ssim_input_output(
    images, vae_outputs, num_images_to_compare, path="CurrentResults/"
)
